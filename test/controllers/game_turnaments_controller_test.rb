...
class GametournamentsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create game_tournament' do
    assert_difference('Gametournament.count') do
      post game_tournaments_url,
           params: { game_tournament: { finalist_id: @game_tournament.finalist_id, name: @game_tournament.name,
                                        status: @game_tournament.status, winner_id: @game_tournament.winner_id } }
    end
  ...
  test 'should update game_tournament' do
    patch game_tournament_url(@game_tournament),
          params: { game_tournament: { finalist_id: @game_tournament.finalist_id, name: @game_tournament.name,
                                       status: @game_tournament.status, winner_id: @game_tournament.winner_id } }
